...
if (isset($_POST["btn_update"])) {
  ...
  if ($_SESSION["username"] == 'admin' || $_SESSION["username"] == 'user') {
    ...
    $q2 = "UPDATE `users` SET `fname`='$fname',`lname`='$lname',`email`='$email' WHERE `email`= '$email'";
  } else {
    $q1 = "UPDATE `customer` SET `fname`='$fname',`lname`='$lname',`email`='$email',`contact`='$contact',`dob`='$dob',`gender`='$gender' where id = '" . $_SESSION["id"] . "'";
    $q2 = "UPDATE `users` SET `fname`='$fname',`lname`='$lname',`email`='$email' WHERE `email`= '$email'";
  }
  //$query1=$conn->query($q1);

  if ($conn->query($q1) === TRUE && $conn->query($q2) === TRUE) {
...
if ($_SESSION["username"] == 'admin' || $_SESSION["username"] == 'user') {
  ...
  $que = "select * from  customer where id = '" . $_SESSION["id"] . "'";
}
              ...
              <form class="form-horizontal" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <div class="row">
                    ...
                    <div class="col-sm-9">
                      <input type="text" value="<?php echo $email; ?>" name="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" class="form-control" readonly required>
                    </div>
                  </div>
                </div>
